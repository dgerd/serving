...
func validateEnv(envVars []corev1.EnvVar) *apis.FieldError {
	...
	for i, env := range envVars {
		errs := errs.Also(validateDisallowedFields(env, *EnvVarMask(&env)).ViaIndex(i))
		if env.ValueFrom != nil {
			errs = errs.Also(validateEnvValueFrom(env.ValueFrom))
		}
	}
	return errs
}
...
func validateEnvFrom(envFromList []corev1.EnvFromSource) *apis.FieldError {
	...
	for i, envFrom := range envFromList {
		...
		if sm != nil {
			errs = errs.Also(validateDisallowedFields(sm, SecretMapEnvSourceMask(sm)).ViaIndex(i))
